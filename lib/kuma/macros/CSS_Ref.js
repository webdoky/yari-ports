...
const typesData = readJsonDependency('mdn-data', 'css/types.json');
...
export default function CSS_Ref(
  ...
  if (types.has('types')) {
    // Add types to index
    for (const [typeName, typeData] of Object.entries(typesData || {})) {
  ...
  if (types.has('at-rules')) {
    ...
    for (const [atRuleName, atRuleData] of Object.entries(atRules || {})) {
  ...
  if (types.has('units')) {
    // Add units to index
    for (const unitTuple of Object.entries(units || {})) {
