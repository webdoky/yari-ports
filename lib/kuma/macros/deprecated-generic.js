...
export default function DeprecatedGeneric(
  ...
  if (obsolescenceVersion) {
    const normalizedObsolescenceVersion = obsolescenceVersion.toLowerCase();
    let n = '';
    if (normalizedObsolescenceVersion.startsWith('js')) {
      n = normalizedObsolescenceVersion.slice(2).trim();
      version = 'JavaScript ';
      if (n.length > 0) {
        link = `/${lang}/docs/JavaScript/New_in_JavaScript/${n}`;
      }
    } else if (normalizedObsolescenceVersion.startsWith('css')) {
      n = normalizedObsolescenceVersion.slice(3).trim();
      version = 'CSS ';
    } else if (normalizedObsolescenceVersion.startsWith('html')) {
      n = normalizedObsolescenceVersion.slice(4).trim();
      const N = n[0] === '4' ? '' : n;
      version = 'HTML';
      link = N.length > 0 ? `/${lang}/docs/HTML${N}` : `/${lang}/docs/HTML`;
  ...
  switch (warningIndicatorType) {
    case 'inline': {
      return `<span class="notecard__deprecated inline" title="${htmlEscape(
        tip,
      )}">${textContent}</span>`;
    }
    case 'header': {
      ...
      return `<div class="notecard__deprecated">
            <h4>${textContent}</h4>
            <p>${description}</p>
        </div>`;
    }
    case 'method': {
      ...
      return `<div>
            <span class="notecard__deprecated inline">${textContent}</span>
            <h3>${extra}</h3>
        </div>`;
    }
    default: {
