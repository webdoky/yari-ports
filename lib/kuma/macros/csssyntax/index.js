...
function cssSyntax(termName, initialAtRule) {
  ...
  function buildLink(match, type) {
    ...
    if (propertyName) {
      ...
      Object.prototype.hasOwnProperty.call(data.externallyDescribedTypes, type)
    ) {
      return (
        '<a href="/' +
        targetLocale +
        data.externallyDescribedTypes[type].link +
        '">&lt;' +
        type +
        '&gt;</a>'
      );
    ...
    return (
      '<a href="' + link + '" title="' + title + '">&lt;' + type + '&gt;</a>'
    );
  }
  ...
  function formatSyntax(rawSyntax) {
    ...
    operators.forEach(function (operator) {
      if (
        formattedSyntax.indexOf('\n') !== -1 &&
        operator.title === 'Curly braces'
      ) {
        return '';
      }
      formattedSyntax = formattedSyntax.replace(
        operator.regexp,
        function (match, text) {
          var linkText = match;
          if (match.indexOf('*') !== -1) {
    ...
    if (typeAnchorAttributes) {
      typeAnchorAttributes.forEach(function (typeAnchorAttribute) {
        ...
        if (
          ...
          if (
            Object.prototype.hasOwnProperty.call(data.syntaxes, subType) &&
            !Object.prototype.hasOwnProperty.call(
              data.externallyDescribedTypes,
              subType
            )
          ) {
            types.push(subType);
