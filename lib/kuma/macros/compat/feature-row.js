...
import nonStandardIcon from '../../../../icons/thumbsdown.js';
import deprecatedIcon from '../../../../icons/delete-bin.js';
import experimentalIcon from '../../../../icons/flask-fill.js';

import { browserName, browserPreviewName } from './browser-name.js';
...
function statusIcons({ status }) {
  ...
  return icons.length === 0
    ...
    : `<div class="bc-icons">
      ${icons
        .map(
          (icon) => `<abbr class="only-icon" title="${icon.title}">
          <span>${icon.text}</span>
          ${icon.icon}
        </abbr>`
        )
...
function featureRow({ index, feature, browsers, activeCell }) {
  ...
  let titleNode;
  const webDocUrl = compat.mdn_url
  ...
  if (compat.bad_url && webDocUrl) {
    ...
    titleNode = `<a href="${webDocUrl}" class="bc-table-row-header">
          ${title}
