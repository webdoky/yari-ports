import atRules from 'mdn-data/css/at-rules.json' assert { type: 'json' };
import selectors from 'mdn-data/css/selectors.json' assert { type: 'json' };
import types from 'mdn-data/css/types.json' assert { type: 'json' };
import properties from 'mdn-data/css/properties.json' assert { type: 'json' };
import syntaxes from 'mdn-data/css/syntaxes.json' assert { type: 'json' };
import units from 'mdn-data/css/units.json' assert { type: 'json' };
import localStrings from 'mdn-data/l10n/css.json' assert { type: 'json' };

import localString from '../utils/local-string.js';
import getLocalString from '../utils/get-local-string.js';
import replacePlaceholders from '../utils/replace-placeholders.js';
import syncReplace from '../utils/sync-replace.js';

import xrefCssInitials from './xref-cssinitial.js';
import xrefCssInherited from './xref-cssinherited.js';
import xrefCssComputed from './xref-csscomputed.js';
...
function cssInfo(termName, initialAtRule) {
  ...
  */
  function addAdditionalAppliesToOutput(output, property, cssInfo) {
    if (property !== 'appliesto' || !Object.hasOwn(cssInfo, 'alsoAppliesTo')) {
      return output;
    }
    ...
    cssInfo.alsoAppliesTo.forEach((element, index, elements) => {
  ...
  */
  function getValueOutput(cssInfo, property, atRule) {
    ...
    if (typeof value === 'string') {
      if (property === 'animationType') {
        ...
        var parsedAnimationTypeValues = animationTypeValues.map(
          (animationTypeValue) => {
            var localizedString = getLocalString(
              localStrings,
              animationTypeValue,
              this.env.targetLocale
            );
        ...
        if (propMatches) {
          ...
          if (property === 'initial' && !Object.hasOwn(localStrings, value)) {
            ...
            var replacedKeywords = keywords.map((keyword) => {
      ...
      var output = localString(value, this.env.targetLocale);
  ...
  if (name === 'preview-wiki-content') {
    ...
    if (Object.hasOwn(cssInfo, 'inherited')) {
      properties = properties.concat({
        name: 'inherited',
    ...
    for (let property of properties) {
      result +=
        ...
        property.label +
        '</th><td>' +
        this.callMacro(getValueOutput, cssInfo, property.name, atRule) +
        '</td>';
    }
    result += '</tbody>' + '</table>';
  } else {
