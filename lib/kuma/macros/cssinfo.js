...
/*
  ...
  */
function addAdditionalAppliesToOutput(output, property, cssInformation) {
  if (
    property !== 'appliesto' ||
    !Object.hasOwn(cssInformation, 'alsoAppliesTo')
  ) {
    return output;
  }
  ...
  const placeholderIndex =
    cssInformation.alsoAppliesTo.indexOf('::placeholder');
  if (placeholderIndex !== -1) {
    cssInformation.alsoAppliesTo.splice(placeholderIndex, 1);
  }

  // In case there are no items left, the output is returned unchanged
  if (cssInformation.alsoAppliesTo.length === 0) {
    return output;
  }

  // eslint-disable-next-line unicorn/no-array-for-each
  cssInformation.alsoAppliesTo.forEach(function (element, index, elements) {
...
function cssInfo(termName, initialAtRule) {
  ...
  const localize = getLocalString.bind({ env: { locale: targetLocale } });
  const localizeString = localString.bind({ env: { locale: targetLocale } });
  ...
  */
  function getValueOutput(cssInformation, property, atRuleParameter) {
    let output;
    if (property === 'relatedAtRule') {
      return `<a href="/${targetLocale}/docs/Web/CSS/${atRuleParameter}"><code>${atRuleParameter}</code></a>`;
    }

    const value = cssInformation[property];
    if (typeof value === 'string') {
      if (property === 'animationType') {
        const animationTypeValues = value.split(' ');
        const parsedAnimationTypeValues = animationTypeValues.map(function (
          animationTypeValue,
        ) {
          let localizedString = localize(localStrings, animationTypeValue);
      ...
      if (propertyMatches) {
        return getValueOutput(data.properties[propertyMatches[1]], property);
      }
      if (property === 'initial' && !Object.hasOwn(localStrings, value)) {
        return `<code>${value}</code>`;
      }
      const keywords = value.split(', ');
      const replacedKeywords = keywords.map(function (keyword) {
    ...
    if (Array.isArray(value)) {
      ...
      return parseMacros(
        addAdditionalAppliesToOutput(output, property, cssInformation),
      );
    }
  ...
  if (name !== 'preview-wiki-content') {
    if (atRule) {
      if (data.atRules[atRule] && data.atRules[atRule].descriptors) {
        cssInformation = data.atRules[atRule].descriptors[name];
      }
    } else if (data.properties[name]) {
      cssInformation = data.properties[name];
    }
  }
  ...
  if (name === 'preview-wiki-content') {
  ...
  } else if (cssInformation) {
    result = '<table class="properties"><tbody>';
    let propertiesHtmlItems = [];
    if (atRule) {
      propertiesHtmlItems = propertiesHtmlItems.concat({
        name: 'relatedAtRule',
    ...
    if (Object.hasOwn(cssInformation, 'inherited')) {
      propertiesHtmlItems = propertiesHtmlItems.concat({
        name: 'inherited',
    ...
    if (cssInformation.percentages !== 'no') {
      // TODO: translate related resources
      // propertiesHtmlItems = propertiesHtmlItems.concat({
      //   name: 'percentages',
    ...
    for (const property of propertiesHtmlItems) {
      result +=
        `<tr>` +
        `<th scope="row">${property.label}</th><td>${getValueOutput(
          cssInformation,
          property.name,
          atRule,
        )}</td>`;
