...
function learningSections({ path: currentPath, targetLocale }) {
  ...
  return [
    {
      ...
      items: firstStepsItems.map((item) => pageToNavItem(item)),
      expanded: firstStepsItems.some(({ path }) => path === currentPath),
    },
    {
      title: labels.CSS_building_blocks,
      items: buildingBlocks.map((item) => pageToNavItem(item)),
      expanded: buildingBlocks.some(({ path }) => path === currentPath),
    },
    {
      title: labels.Styling_text,
      items: stylingText.map((item) => pageToNavItem(item)),
      expanded: stylingText.some(({ path }) => path === currentPath),
    },
    {
      title: labels.CSS_layout,
      items: cssLayout.map((item) => pageToNavItem(item)),
      expanded: cssLayout.some(({ path }) => path === currentPath),
    },
  ];
}
